...
namespace Gists
{
    ...
    public static class FastPoissonDiskSampling
    {
        ...
        public static List<Vector2> Sampling(Vector2 bottomLeft, Vector2 topRight, float minimumDistance, int iterationPerPoint)
        {
            ...
            do
            {
                var index = ThreadSafeRandom.Range(0, bags.ActivePoints.Count);
        ...
        private static void GetFirstPoint(Settings set, Bags bags)
        {
            var first = new Vector2(
                ThreadSafeRandom.Range(set.BottomLeft.x, set.TopRight.x),
                ThreadSafeRandom.Range(set.BottomLeft.y, set.TopRight.y)
            );
        ...
        {
            var theta = ThreadSafeRandom.Range(0f, Mathf.PI * 2f);
            var radius = Mathf.Sqrt(ThreadSafeRandom.Range(fieldMin * fieldMin, fieldMax * fieldMax));
